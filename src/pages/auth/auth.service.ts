...
export function useAuthService({ onAuthenticated }: AuthServiceOptions = {}) {
  ...
  const onLogin = useCallback(
    async (values: LoginInputs) => {
      try {
        ...
        toast.error(
          error instanceof Error ? error.message : t("toast.signInError"),
        );
      }
    },
  ...
  const onRegister = useCallback(
    async (values: RegisterInputs) => {
      try {
        ...
        toast.error(
          error instanceof Error ? error.message : t("toast.accountError"),
        );
      }
    },
    [loginForm, t],
  ...
  const onGoogleSignIn = useCallback(async () => {
    try {
      ...
      console.log(response, "res google");
      if (response.url) {
        window.location.assign(response.url);
      }
    } catch (error) {
      toast.error(
        error instanceof Error ? error.message : t("toast.googleStartError"),
      );
    }
  }, [t]);
